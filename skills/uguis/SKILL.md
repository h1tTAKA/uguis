...
| "속도 50", "말속도 60으로", "rate 55", "우구이스 50", "더 느리게/빠르게" | `rate <숫자>` |
| "쉼 1", "pause 2", "쉬는거 늘려/줄여", "정지 없이" | `pause <0~2>` |
| "우구이스 상태", "지금 켜져있어?" | `status` |
| 그냥 "/uguis" | `toggle` |

- `shush` = **지금 나오는 재생만** 멈춤(음성 기능은 켜진 채, 다음 답변은 정상). `off`(음소거 지속)와 다름.
- **replay·stop은 스킬이 아니라 훅**이다(`tts-replay-hook.py`, UserPromptSubmit). 평문으로:
  - **`re`** / "다시 읽어줘" → 바로 위 답변 다시 재생
  - **`stop`** / "멈춰" / "그만" → 지금 재생 즉시 중단(shush)
  클로드 경유 없이(챗터·새 턴 없음) 처리된다. 이 정확 키워드들은 스킬로 처리하지 말 것.
- `rate <숫자>` = 말속도 `+<숫자>%`로(진행·최종 동시). 예: `rate 50`. **재시작 없이 다음 발화부터** 반영(config 파일 실시간 읽음).
- `pause <0~2>` = 문장부호에서 쉬는 정도. `0`=정지 없음(기본), `1`=`,.:`에서 약간, `2`=절 사이까지 많이.
- "더 느리게/빠르게"처럼 상대적 요청이면 현재값(`~/.claude/.tts-config`의 `rate`, 없으면 기본 60)을 기준으로 ±10 정도 조절해 `rate <숫자>` 실행.
